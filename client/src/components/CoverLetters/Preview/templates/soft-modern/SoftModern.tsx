...
export default function SoftModern({ personalDetails, coverLetter, theme }: CoverLetterTemplateProps) {
  const appliedFont =
    theme.fontFamily ||
    'Georgia, ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, "Apple Color Emoji", "Segoe UI Emoji"';
  const today = formatToday(theme.dateLocale || 'en-US', 'full');

  return (
    <div
      className="cl-html-root cl-soft-modern"
    ...
    >
      <div data-cl-block="header" className="cl-header">
        <div className="cl-header-name" style={{ color: theme.primaryColor }}>{personalDetails.fullName || 'Your Name'}</div>
        <div className="cl-header-contact">
          ...
          {personalDetails.linkedin ? <span className="cl-dot"> | </span> : null}
