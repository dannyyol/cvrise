...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column('default_length', sa.String(length=32), nullable=False),
        sa.Column('guidelines', sa.JSON(), nullable=False),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('key')
    )
    with op.batch_alter_table('cover_letters', schema=None) as batch_op:
        batch_op.add_column(sa.Column('template_key', sa.String(length=64), nullable=False, server_default='professional'))
        batch_op.add_column(sa.Column('tone', sa.String(length=32), nullable=False, server_default='professional'))
        batch_op.add_column(sa.Column('length', sa.String(length=32), nullable=False, server_default='medium'))
